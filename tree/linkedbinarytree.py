...
# preorder is tycally used to express hierarchy (first root, left, right)
def preorder_indent (T, p, d):
    """a preorder traversal to print out 
    hierachical structure of the tree"""
    print (2*d*' ' + str(p.element()))
    for child in T.children(p):
        preorder_indent(T, child, d+1)
        

# tests on LinkedBinaryTree class

liltree = LinkedBinaryTree()
root = liltree._add_root('root')
print (liltree.parent(root))

left1 = liltree._add_left(root, 'left1')
right2 =liltree._add_right(root, 'right1')
left1_l = liltree._add_left(left1, 'left1_left2')
left1_r = liltree._add_right(left1, 'left1_right2')
print (len(liltree))
print (liltree.num_children(root))
# replace root with new root
print (liltree._replace(root, 'new root'))
print (liltree.root().element())


print ("tree traversal in preorder: ")
# I can use regular for loop to iterate because 
# I implemented the __iter__ in the base tree class
# the __iter__ indicated that the order of traversal is preorder
for element in liltree:
    print (element)

print("tree traversal in postorder: ")
for position in liltree.postorder():
    print (position.element())

print("tree traversal in breadth first order: ")
for position in liltree.breadthfirst():
    print (position.element())

print("tree traversal inorder: ")
for position in liltree.inorder():
    print (position.element())

print ("depth of root:")
print (liltree.depth(root))
print ("height of root:")
print (liltree.height(root))

# printing out the hierarchy of the tree
print("\npreorder traversal printout:")
preorder_indent(liltree, liltree.root(), 0)
